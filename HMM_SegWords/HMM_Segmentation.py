...
def save_parameter(pi, A, B, time):
    f_pi = open(".\\data\\pi%d.txt" % time, "w")
    list_write(f_pi, pi)
    f_pi.close()
    f_A = open(".\\data\\A%d.txt" % time, "w")
    for a in A:
        list_write(f_A, a)
    f_A.close()
    f_B = open(".\\data\\B%d.txt" % time, "w")
    for b in B:
        list_write(f_B, b)
    f_B.close()
...
def load_train():
    ...
    for line in f:
        pi = list(map(float, line.strip().split(' ')))  # pi.txt最后一行是空行
    f.close()
    ...
    i = 0
    for line in f:
        A[i] = list(map(float, line.strip().split(' ')))
        i += 1
    f.close()
    ...
    i = 0
    for line in f:
        B[i] = list(map(float, line.strip().split(' ')))
        i += 1
    f.close()
    return pi, A, B
...
if __name__ == "__main__":
    pi, A, B = load_train()
    f = open(".\\data\\24.mybook.txt", encoding="utf-8")
    data = f.read()[3:]
    f.close()
