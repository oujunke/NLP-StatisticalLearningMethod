...
def baum_welch(pi, A, B):
    f = open(".\\data\\1.txt", encoding="utf-8")
    sentence = f.read()[3:]
    f.close()
    T = len(sentence)
...
def mle():  # 0B/1M/2E/3S
    ...
    f = open(".\\data\\24.pku_training.utf8", encoding="utf-8")
    data = f.read()[3:]
    f.close()
    tokens = data.split('  ')
    last_q = 2
...
def save_parameter(pi, A, B):
    ...
    list_write(f_pi, pi)
    f_pi.close()
    f_A = open(".\\data\\A.txt", "w", encoding="utf-8")
    for a in A:
        list_write(f_A, a)
    f_A.close()
    f_B = open(".\\data\\B.txt", "w", encoding="utf-8")
    for b in B:
        list_write(f_B, b)
    f_B.close()
...
if __name__ == "__main__":
    pi, A, B = mle()  # Maximum likelihood estimate
